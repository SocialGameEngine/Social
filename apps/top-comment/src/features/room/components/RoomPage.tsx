...
function RoomPageContent() {
  ...
  return (
    <div className="min-h-[90dvh] flex flex-col bg-gradient-to-b from-slate-900 to-slate-800 text-white sm:overflow-hidden">
      ...
      <nav className={`bottom-nav sm:hidden ${shouldHideBottomNav ? 'hidden' : ''}`}>
        <button type="button" className="nav-item" onClick={() => window.location.href = '/join'}>
          <div className="text-2xl">🚪</div>
          <span className="nav-label">Bail</span>
        </button>
        <button
          type="button"
          className={`chaos-nav-item ${showLobbyDrawer ? 'opacity-100' : 'opacity-70'}`}
