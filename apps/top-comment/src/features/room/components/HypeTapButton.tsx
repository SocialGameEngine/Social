...
interface HypeTapButtonProps {
  ...
  active: boolean;
  /** Optional player count for scaling hype thresholds. */
  playerCount?: number;
}
 ...
 */
export function HypeTapButton({ roomId, active, playerCount }: HypeTapButtonProps) {
  const { snapshot, tap } = useHypeMeter({ roomId, enabled: active, playerCount });
    ...
    <AnimatePresence>
      {active && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className="fixed inset-x-0 bottom-6 z-[62] flex flex-col items-center gap-2 px-6"
        >
