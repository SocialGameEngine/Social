...
export function LeaderboardModal({
  ...
  const scrollToMyRank = useCallback(() => {
    if (scoreboardRef.current) {
      // Find the element for current team and scroll to it
  ...
  const downloadSelfie = useCallback(() => {
    ...
    link.download = 'vibox-selfie.png';
    link.click();
  }, [selfieImage]);

  if (!isOpen) return null;

  const currentTeam = finalLeaderboard.find(t => t.id === currentMembershipId);
        ...
        <div className="space-y-5 p-4">
          ...
          >
            <Leaderboard
              ...
              highlightTeamId={currentMembershipId}
