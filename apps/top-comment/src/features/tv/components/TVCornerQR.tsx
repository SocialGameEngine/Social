...
/**
 * P1-26 — persistent top-right QR that stays mounted for the whole /tv session.
 *
 * Uses `layout` animations so the transition between expanded (idle) and
 * shrunken (active play) is smooth instead of a pop. Positioned in the top-
 * right corner so it never collides with the bottom-left player-count badge or
 * the bottom hype meter.
 */
export function TVCornerQR({ joinUrl, roomCode, isActivePlay }: TVCornerQRProps) {
  return (
    <motion.div
      ...
      className="fixed top-4 right-4 z-40 md:top-6 md:right-6 pointer-events-none"
      style={{
