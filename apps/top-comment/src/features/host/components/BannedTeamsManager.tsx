...
export function BannedPlayersManager({ 
  ...
  const handleUnbanPlayer = async (bannedPlayer: BannedPlayer) => {
    ...
    try {
      const { error } = await supabase.functions.invoke('top-comment-sessions-unban-player', {
        body: {
          sessionId: sessionId,
          bannedTeamId: bannedPlayer.id
        }
      });
              ...
              <Card key={bannedPlayer.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <h3 className="font-medium">{bannedPlayer.display_name || bannedPlayer.user_id}</h3>
